...
set -euo pipefail
cd "$(dirname "$0")/.."
python3 scripts/1_fetch_listings.py     # pull live CARFAX inventory -> raw/listings.json
python3 scripts/2_score_listings.py     # compute sub-scores          -> clean.json
...
python3 scripts/recalc.py Bella_Car_Search.xlsx 400
